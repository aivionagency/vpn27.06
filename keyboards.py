...
def start_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.button(text="🟢 Подключить", url="https://example.com/placeholder")
    builder.button(text="🔹 Инструкция", callback_data=ActionCallbackData(action="instruction").pack())
...
def main_menu_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="🔹 Мой VPN", callback_data=ActionCallbackData(action="my_subscription").pack())
...
def subscription_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="🟢 Подключить / Обновить ключ", url="https://example.com/placeholder")
    builder.button(text="🟢 Продлить", callback_data=ActionCallbackData(action="buy").pack())
    builder.button(text="🔹 Пригласить друга", callback_data=ActionCallbackData(action="referral").pack())
    builder.button(text="Назад", callback_data=ActionCallbackData(action="main_menu").pack())
    builder.adjust(1)
    return builder.as_markup()
...
def tariffs_keyboard(devices: int) -> InlineKeyboardMarkup:
    ...
    builder.button(text="Назад", callback_data=ActionCallbackData(action="buy").pack())
    builder.adjust(1)
    return builder.as_markup()
...
def tariff_confirmation_keyboard(tariff_id: int, devices: int) -> InlineKeyboardMarkup:
    ...
    builder.button(text="Назад", callback_data=DeviceCallbackData(count=devices).pack())
    builder.adjust(1)
    return builder.as_markup()

def successful_purchase_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="🔹 Моя подписка", callback_data=ActionCallbackData(action="my_subscription").pack())
